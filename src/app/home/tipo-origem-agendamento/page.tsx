...
export default function TipoOrigemAgendamentoPage() {
    const [tipoOrigemAgendamento, setTipoOrigemAgendamento] = useState<TipoOrigemAgendamento[] | null>();
    ...
    return (
       <main className='flex flex-col items-center justify-center gap-5 min-h-screen h-full w-full'>
        <h1 className="font-semibold text-2xl text-slate-700">Tipo Origem Agendamento</h1>
           ...
           <ul className='flex flex-col gap-2'>
             {tipoOrigemAgendamento &&
               tipoOrigemAgendamento.map((tipoOrigemAgendamento) => (
                 <div key={tipoOrigemAgendamento.nu_tp_origem_agendamento} className='flex justify-between w-full'>
                   <li className='text-black text-sm'>{tipoOrigemAgendamento.no_tipo}</li>
   
                   <div className='flex gap-2'>
                     <MdEdit
                       onClick={() =>
                         router.push(`/home/tipo-origem-agendamento/${tipoOrigemAgendamento.nu_tp_origem_agendamento}/edit`)
                       }
